"use strict";
(() => {
  // src/artifacts/notes-system-project-dashboard.js
  (async function initProjectDashboard() {
    const isProjectHub = (note) => note && (note.hasLabel?.("extProjectHub") || note.hasLabel?.("extTemplate", "projectHub") || note.hasLabel?.("noteType", "projectHub"));
    let hub;
    let container;
    let context;
    ...
    const isProjectHubContext = hub && (hub.hasLabel?.("extProjectHub") || hub.hasLabel?.("extTemplate", "projectHub") || hub.hasLabel?.("noteType", "projectHub"));
    ...
    const labelValue = (note, name) => note?.getOwnedLabelValue?.(name) ?? note?.labels?.find?.((label) => label.name === name)?.value ?? "";
    ...
    const isDone = (note) => Boolean(labelValue(note, "doneDate")) || ["done", "complete", "completed"].includes(labelValue(note, "status").toLowerCase());
    ...
    const loadDashboard = async () => {
      ...
      const rounds = notes.filter((note) => noteKind(note) === "round").sort((a, b) => Number(labelValue(a, "round") || 0) - Number(labelValue(b, "round") || 0));
      ...
      const meetings = notes.filter((note) => noteKind(note) === "meeting").sort((a, b) => noteDate(a).localeCompare(noteDate(b)));
      const communications = notes.filter((note) => ["email", "reporting"].includes(noteKind(note)));
      const followups = notes.filter((note) => labelValue(note, "status") === "awaiting" || labelValue(note, "followUpDate") && !isDone(note));
      const currentRound = rounds.length ? rounds[rounds.length - 1] : null;
      panel.querySelector("[data-project-current-round]").textContent = currentRound ? `Current round: ${labelValue(currentRound, "round") || "\u2014"} \u2014 ${currentRound.title}` : "Current round: none yet.";
      ...
      renderSection("rounds", makeTable(["Round", "Story", "Status"], [...rounds].reverse().map((note) => [
        labelValue(note, "round"),
        makeLink(note),
        makeStatus(labelValue(note, "status"))
      ...
      renderSection("followups", makeTable(["Note", "Status / follow-up"], followups.map((note) => [
        makeLink(note),
        `${labelValue(note, "status") || "open"}${labelValue(note, "followUpDate") ? ` \xB7 ${labelValue(note, "followUpDate")}` : ""}`
      ])), followups.length > 0);
    };
