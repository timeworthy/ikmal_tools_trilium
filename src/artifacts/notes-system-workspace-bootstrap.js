/*
 * Repairs the small amount of note-tree wiring that cannot be represented by
 * a package artifact alone: the visible Today entry and project dashboards.
 *
 * This is deliberately idempotent. It only creates a missing note or relation
 * owned by Ikmal; daily-note cleanup only removes the package-generated
 * structural sections while preserving any text the user entered there.
 */
...
(function initWorkspaceBootstrap() {
    ...
    function packageCode(artifactId, notes) {
        return notes.find((note) => note.type === 'code'
            && [artifactId, `${artifactId}-script`].includes(note.getOwnedLabelValue('packageArtifact'))
            && !note.isArchived);
    }
    ...
    function hasMarker(note, name, value) {
        ...
        return marker !== undefined && marker !== null && (value === undefined || marker === value);
    }
    ...
    async function removeProjectDashboardsFromDailyNotes() {
        ...
        for (const dailyNote of dailyNotes) {
            ...
            for (const child of children) {
                const isProjectDashboard = hasMarker(child, 'extProjectDashboard', 'projectHub')
                    || hasMarker(child, 'extHubDashboard', 'projectHub');
                if (!isProjectDashboard || child.isArchived) continue;
                const parentIds = child.getParentNoteIds?.() || [];
                // A dashboard belongs under a project hub. Archive only the
                // stray package-created note that has no other active parent,
                // preserving a legitimate dashboard if it is also attached
    ...
    async function attachProjectDashboards(dashboardCode) {
        ...
        for (const project of projects) {
            ...
            const hydratedProject = typeof api.getNote === 'function'
                ? await api.getNote(project.noteId)
                : project;
            const children = hydratedProject?.getChildNotes
                ? await hydratedProject.getChildNotes()
                : [];
    ...
    async function ensureSkeletonContainers() {
        ...
        const dailyTpl = (await searchIncludingHidden('#extTemplate'))
            .find((n) => markerValue(n, 'extTemplate') === 'daily' || n.title === 'Daily Note');
        if (journal && dailyTpl) {
            const hasDateTpl = journal.getRelations?.('dateTemplate')?.some((r) => (r.value || r.targetNoteId) === dailyTpl.noteId);
            if (!hasDateTpl) {
                try { await setAttribute(journal.noteId, 'relation', 'dateTemplate', dailyTpl.noteId); } catch {}
            }
        }
    }
    ...
    async function migrateEditRoundBodies() {
        const storyNotes = (await searchMany(['#extStoryDraft', '#extTemplate']))
            .filter((n) => !n.isArchived && (hasMarker(n, 'extStoryDraft') || hasMarker(n, 'extTemplate', 'storyDraft')));
        let migrated = 0;
        const legacyBodies = [
    ...
    async function reconcileProjectHubStatuses() {
        ...
        for (const hub of hubs) {
            ...
            const drafts = children
                .filter((c) => hasMarker(c, 'extStoryDraft') || hasMarker(c, 'extTemplate', 'story') || hasMarker(c, 'extTemplate', 'storyDraft'))
                .sort((a, b) => Number(markerValue(b, 'round') || 0) - Number(markerValue(a, 'round') || 0));
            ...
            if (isLatestDone && status !== 'complete') {
                ...
                if (archiveRoot?.noteId) {
                    try { await cloneNoteToParent(hub.noteId, archiveRoot.noteId); } catch {}
                }
                ...
                if (activeRoot?.noteId) {
                    try { await cloneNoteToParent(hub.noteId, activeRoot.noteId); } catch {}
                }
                if (archiveRoot?.noteId) {
                    try {
    ...
    async function ensureBackendEventWiring() {
        const backendMetadataSync = (await searchIncludingHidden('#packageArtifact'))
            .find((n) => n.getOwnedLabelValue?.('packageArtifact') === 'notes-system-project-metadata-sync');
        const backendDailyRepair = (await searchIncludingHidden('#packageArtifact'))
        ...
        if (backendDailyRepair) {
            ...
            if (journal) {
                for (const relName of ['runOnNoteCreation', 'runOnNoteChange']) {
                    ...
                    if (!hasRel) {
                        try { await setAttribute(journal.noteId, 'relation', relName, backendDailyRepair.noteId); } catch {}
                    }
                }
            }
        }
        ...
        if (backendTopicSync) {
            ...
            for (const marker of topicRoots) {
                ...
                for (const relName of ['runOnAttributeCreation', 'runOnAttributeChange', 'runOnNoteCreation', 'runOnNoteChange']) {
                    ...
                    if (!hasRel) {
                        try { await setAttribute(rootNote.noteId, 'relation', relName, backendTopicSync.noteId); } catch {}
                    }
                }
            }
        }
    ...
    async function syncProjectMetadata() {
        ...
        for (const hub of hubs) {
            ...
            const drafts = children
                .filter((c) => hasMarker(c, 'extStoryDraft') || hasMarker(c, 'extTemplate', 'story'))
                .sort((a, b) => Number(markerValue(b, 'round') || 0) - Number(markerValue(a, 'round') || 0));
            const latestRound = drafts[0];

            // Auto-rename Reporting Notes if hub title changed and extReportingTitleManaged is set
            ...
            // Sync latest round number back to hub currentRound label
            if (latestRound) {
                const roundNum = markerValue(latestRound, 'round');
                if (roundNum && markerValue(hub, 'currentRound') !== String(roundNum)) {
                    try { await setAttribute(hub.noteId, 'label', 'currentRound', String(roundNum)); } catch {}
                }
            }

            // Bidirectional relation and override sync across hub, latest round, and reporting notes
                ...
                if (relVal) {
                    for (const n of group) {
                        ...
                        if (!(cur?.value || cur?.targetNoteId)) {
                            try { await setAttribute(n.noteId, 'relation', relationName, relVal); } catch {}
                        }
                    }
                }
                ...
                if (overrideVal) {
                    for (const n of group) {
                        if (!markerValue(n, overrideName)) {
                            try { await setAttribute(n.noteId, 'label', overrideName, String(overrideVal)); } catch {}
                        }
                    }
                }
            }
        ...
        for (const s of searches) {
            ...
            if (existing && existing.length > 0) {
                ...
                if (markerValue(note, 'searchString') !== s.search) {
                    try { await setAttribute(note.noteId, 'label', 'searchString', s.search); } catch {}
                }
                continue;
            }
            try {
    ...
    async function repair() {
        // Search the broad marker first, then filter exact values in JS. The
        ...
        const todayCode = packageCode('notes-system-today-page', todayPageNotes)
            || packageCode('notes-system-dashboard', todayNotes);
